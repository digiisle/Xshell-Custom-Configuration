def Main():
    ...
    logFileName=str("AutoLog_"+datetime.datetime.now().strftime("%Y-%m-%d_%H%M%S")+".log")
   
    #*** Start log***
    xsh.Session.LogFilePath = (Path+logFileName)
    xsh.Session.StartLog()
    #*** Send command***
    for line in open("C:\\Users\\qiany\\Documents\\NetSarang Computer\\7\\Xshell\\Scripts\\CommandManager\\AutoConfig.txt"):
